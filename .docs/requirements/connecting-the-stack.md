...
None of these is blocked by the contract. `SubstitutionEnvelope` carries
`leaves_unfilled`, and the catalogue and install envelopes carry the rest, so
each is work not yet done rather than a gap to be raised against the SDK.
