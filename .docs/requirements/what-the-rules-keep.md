...
| `N1-R47` | Pairing material can be produced on demand, from a surface the operator is already admitted to, without the app being reachable | The stack's side. It is an `N1` row because it is about pairing *this* app, and it is here because the whole of it is a thing the stack must be able to do while the app is absent — which is precisely the state this app cannot test from. What depends on it is that `PairByScanning` and `PairByTyping` never have to ask the stack for a code: the operator brings one |
