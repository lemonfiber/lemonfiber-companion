...
`N11-R3`, `N11-R9`, `N11-R10`); `E3` is the list of copies, with an empty one told apart from one that could not be read (`N6-R9`); `C4` is a support bundle, chosen, described before it is written and read in full here, with a refusal drawn as one (`N22-R1` to `N22-R8`, `N22-R10`); `G8` is what leaves the machine, ours and
