...
/**
 * A stack where a test says what its front door is, and which remembers being asked.
 *
 * {@see AStackThatChecksItself}' sibling one endpoint along.
 *
 * Not `readonly`: what was asked is written when the asking happens.
 */
final class AStackWithAFrontDoor implements Welcoming
{
    ...
    /** @param Closure(): WhatWasFoundOfTheFrontDoor $answer */
    private function __construct(private readonly Closure $answer) {}

    /** A stack with this door. */
    public static function with(TheFrontDoor $door): self
    {
        return new self(static fn(): WhatWasFoundOfTheFrontDoor => WhatWasFoundOfTheFrontDoor::found($door));
    }

    /** A stack the operator could not reach, for the reason given. */
    public static function met(Obstacle $why): self
    {
        return new self(static fn(): WhatWasFoundOfTheFrontDoor => WhatWasFoundOfTheFrontDoor::met($why));
    }
