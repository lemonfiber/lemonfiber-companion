...
it('R3 — an exemption for the tests names both places the tests live', function (): void {
    // This repository keeps its tests in two homes: the root suites under
    // `tests/`, and each module's own under `app-modules/<name>/tests`. Both
    // are testsuites in `phpunit.xml` and both are analysed.
    //
    // A `disallowed-calls` exemption that names only `tests/*` therefore holds
    // half the suite to the rules written for production code, while its own
    // message says "the tests are the exception". All nineteen of them did,
    // and nothing said so: a module test that needs a clock, a filesystem or
    // reflection simply fails the analyser, and the cure looks like rewriting
    // the test rather than like a path that was written against one of two
    // directories.
    //
    // The same shape as a gate reading one manifest of thirteen, and it fails
    // the same way — quietly, on the half nobody was looking at.
    $configuration = file_get_contents(Tree::at('phpstan.neon'));
    ...
    foreach ($lines as $number => $line) {
        ...
        $next = trim($lines[$number + 1] ?? '');

        if ($next !== '- app-modules/*/tests/*') {
            $lonely[] = sprintf('phpstan.neon:%d', $number + 1);
        }
    }

    expect($lonely)->toBe([], sprintf(
        "These exempt the tests and name only one of the two places tests live:\n  %s\n\n"
        . 'Add `- app-modules/*/tests/*` beneath it. A module\'s tests are tests: they '
        . 'are a testsuite in phpunit.xml, they are in the analyser\'s paths, and every '
        . 'one of these messages already says the tests are the exception (R3).',
        implode("\n  ", $lonely),
    ));
});
