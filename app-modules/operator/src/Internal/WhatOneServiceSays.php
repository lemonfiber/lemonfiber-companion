 ...
 * same reason: {@see Daemon} hands one of its facts over through a closure and
 * Blade has no way to call one, so the fold happens once per row here.
 *
 ...
 * handed back to {@see WhereAStackIs::logsOf()} —
...
final readonly class WhatOneServiceSays
{
     ...
     * {@see WhatOneStalledItemSays::in()}'s reason: one closure builds the
     * whole row, so a field cannot reach a template by a path that skipped the
     * value object.
     */
    public static function in(Daemon $daemon): self
    {
        $leaning = [];
     ...
     */
    private static function exited(Daemon $daemon): string
    {
        return $daemon->exit(
